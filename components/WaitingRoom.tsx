...
export default function WaitingRoom({
  ...
  // Extract variables safely from your JSON structure
  const actualRoom = roomData?.room || roomData?.gameState || roomData?.data || roomData;

  const modes: GameModeOption[] = actualRoom?.availableModes || [];
  const roomId = actualRoom?.roomId || "";
  const roomType = actualRoom?.type || "";
  const playerMap = actualRoom?.playerMap || {};
  const hostInfo = actualRoom?.host;

  // Strict host verification matching your WS payloads
  const amIActuallyHost = 
    hostInfo?.id === currentPlayerId || 
    playerMap[currentPlayerId]?.host === true;
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-white p-4 font-sans">
      <div className="bg-zinc-950 border border-zinc-800 rounded-[40px] p-8 md:p-12 w-full max-w-xl shadow-2xl text-center relative overflow-hidden">
        ...
        <header className="relative z-10">
          ...
          {/* ROOM CODE DISPLAY WITH THE INTEGRATED COPY TRIGGER */}
        ...
        {/* LOBBY HOST INTERFACE - LIGHTWEIGHT & REDUCED COMPACT SIZING */}
        ...
        {/* CONNECTED PLAYERS LIST CONTAINER - EXPANDED CAPACITY FOR 4-5 PEOPLE */}
        <div className="relative z-10 space-y-3 mb-8 text-left">
          ...
          <div className="space-y-2 max-h-[260px] overflow-y-auto pr-2 custom-scrollbar">
            {Object.values(playerMap).map((player: any) => {
              const isMe = player.id === currentPlayerId;
              const isThisPlayerHost = player.host === true;
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-black text-xs uppercase tracking-wide text-zinc-300 group-hover:text-cyan-400 transition-colors">
                      {player.name || "Joining..."} {isMe && <span className="text-[10px] text-zinc-500 lowercase italic ml-1">(you)</span>}
                    </span>
                  </div>
                  {isThisPlayerHost && (
        ...
        {/* BOTTOM TRIGGER DISPATCH BAR - CONDITIONAL ACTION LOCK */}
