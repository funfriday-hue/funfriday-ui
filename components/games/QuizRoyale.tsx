...
type GameData = { question?: string; category?: string; questionType?: "LIST" | "CHRONOLOGY"; playMode?: "ALL_PLAY" | "ROUND_ROBIN"; acceptedAnswers?: string[]; currentPlayerId?: string; timeoutCoordinatorId?: string; allPlayAnsweredPlayerIds?: string[]; chronologyPassedPlayerIds?: string[]; turnStartedAtMillis?: number; turnSeconds?: number; strikeLimit?: number; lastEvent?: string; chronologyHint?: string; finished?: boolean };
...
export default function QuizRoyale({ roomId, playerId, playerName, stompClient, publicState, synchronizedPlayers }: { roomId: string; playerId: string; playerName: string; stompClient: StompClient; publicState: unknown; synchronizedPlayers: Player[] }) {
  ...
  const strikeCount = strikePlayer?.stats?.strikes ?? 0;
  ...
  return <div className="h-full overflow-y-auto bg-[#090a0f] px-5 pb-6 pt-28 text-white"><div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-[1fr_320px]">
    <section className="rounded-[2rem] border border-white/10 bg-zinc-950 p-6 shadow-2xl">
      ...
      <div className="my-7 rounded-3xl border border-white/10 bg-white/[.03] p-6 text-center"><p className="text-[10px] font-mono uppercase tracking-[.3em] text-zinc-500">{isAllPlay ? "All Play" : "Current turn"}</p><p className="mt-2 text-3xl font-black text-cyan-300">{isAllPlay ? "Answer now" : activePlayer ? `${activePlayer.name}'s turn` : "Waiting…"}</p><motion.p key={`${strikePlayer?.id || "player"}-${strikeCount}`} initial={{ scale: 1.35, color: "#fb7185" }} animate={{ scale: 1, color: "#a1a1aa" }} transition={{ type: "spring", stiffness: 420, damping: 16 }} className="mt-2 text-sm font-bold">Strikes: {strikeCount}/{data.strikeLimit || 3}</motion.p><p className="mt-2 text-sm text-zinc-400">{isMyTurn ? (isAllPlay ? "Keep submitting distinct correct answers." : "Submit an answer.") : isAllPlay ? "You are eliminated from this match." : "Only the active player can answer."}</p></div>
      <form onSubmit={submit} className="flex gap-3"><input value={answer} onChange={event => setAnswer(event.target.value)} disabled={!isMyTurn || isFinished} placeholder={isMyTurn ? "Type your answer…" : "Wait for your turn"} className="min-w-0 flex-1 rounded-2xl border border-white/10 bg-black px-5 py-4 text-white outline-none focus:border-cyan-400 disabled:opacity-50"/><button disabled={!isMyTurn || isFinished} className="rounded-2xl bg-cyan-400 px-6 py-4 text-xs font-black uppercase tracking-widest text-black disabled:opacity-40">Submit</button>{(!isAllPlay || data.questionType === "CHRONOLOGY") && <button type="button" onClick={passTurn} disabled={!isMyTurn || isFinished || (isAllPlay && hasPassedCurrentChronologyItem)} className="rounded-2xl border border-rose-400/50 px-5 py-4 text-xs font-black uppercase tracking-widest text-rose-300 transition-colors hover:bg-rose-400/10 disabled:opacity-40">{isAllPlay && hasPassedCurrentChronologyItem ? "Passed" : "Pass"}</button>}</form>
      {data.lastEvent && <p className="mt-5 text-center text-sm font-bold text-zinc-400">{data.lastEvent}</p>}
    </section>
    <aside className="flex max-h-[420px] flex-col rounded-[2rem] border border-white/10 bg-zinc-950 p-5"><h2 className="text-[10px] font-mono uppercase tracking-[.3em] text-zinc-500">Answers found</h2><div ref={answersListRef} className="mt-4 min-h-0 space-y-3 overflow-y-auto pr-2">{(data.acceptedAnswers || []).map((accepted, index) => <p key={`${index}-${accepted}`} className="text-sm font-semibold text-white">{index + 1}. {accepted}</p>)}{!(data.acceptedAnswers || []).length && <p className="text-sm text-zinc-600">No accepted answers yet.</p>}</div></aside>
  </div><ResultModal isOpen={isFinished} title="Quiz Royale complete" players={sortedPlayers} localPlayerName={playerName} localPlayerId={playerId} renderStats={player => <div><p className="font-black text-cyan-300">{player.score} points</p><p className="text-xs text-zinc-500">{player.stats?.strikes || 0}/{data.strikeLimit || 3} strikes</p></div>} /></div>;
}
